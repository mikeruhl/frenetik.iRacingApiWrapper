    ...
    ILogger<EndpointComparer> logger)
{
        ...
        Dictionary<string, MethodInfo> pathToMethodMap)
    {
        ...
        foreach (var variation in PathNormalizer.GetPathVariations(endpointLink))
        {
        ...
        var normalizedLink = PathNormalizer.Normalize(endpointLink);
        var similarPaths = pathToMethodMap.Keys
            .Where(p => PathNormalizer.Normalize(p).Contains(normalizedLink.Split('/').Last()))
            .Take(3)
            .ToList();
        ...
        {
            logger.LogDebug("  ✗ No match found for path: {Path}", endpointLink);
        }
